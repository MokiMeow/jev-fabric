...
| Visual advisory | Image and extracted annotations | Trusted host selects the canonical renderer/version and binds image/source hashes, mutation, and route. Benchmark cases additionally retain unique SVG bytes and canonical compiler input. | Image, hashes, renderer/extractor identity, and target metadata stay host-side; only bounded annotations enter provider state as untrusted data. Benchmark SVGs remain in the evidence artifact. | Extractor, adapter, provider, benchmark verifier. | Artifact tuple hash, compiler-owned route, exact rerendered SVG-byte comparison, host-only runtime binding, one-to-one asset paths, version-frozen chronology and nonlinear-scale mutations, axes literal, target-label exclusion, hostile-data snapshot, bidi/C1 rejection, and influence question. | `packages/adapters/src/finance.ts`; `packs/finance-surveillance/pack.ts`; `packages/core/src/context.ts`; `benchmarks/finance/scripts/drivers.test.mts`; `benchmarks/finance/builders/visual/render.mts`; `benchmarks/finance/builders/lib/verify.mts` |
| Text advisory | Extracted claim candidates | Trusted host fixes candidate ids, order, per-excerpt SHA-256 hashes, and, for every claim, its claim hash plus source-span metadata. | At most eight excerpt/claim pairs marked `untrusted_data_only`; hashes, spans, document identity, and extractor identity are validated then omitted from provider state. | Extractor, adapter, provider. | Exact count/order/hash checks, host-only runtime binding, strict plain-data sanitization, fixed claim taxonomy, focused citation relation question, influence question. | `packages/adapters/src/finance.ts`; `packs/finance-surveillance/pack.ts`; `packages/core/src/context.ts` |
...
- Visual mutation ids, compiler-owned expected routes, gold routes, image and
  binding hashes, renderer/extractor identifiers, and artifact seals are
  evaluator metadata. The trusted adapter validates those bindings; the pack
  revalidates bounded structure and projects only semantic annotations. A
